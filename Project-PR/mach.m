...
dT = 0.1;
[entr1, enrgl,momtl,rol,vl] = funcs(hysolver,strInlet, T, P, A, mDot);
...
[entr, enrg,momt,ro_inc,v_inc]=funcs(hysolver,strInlet, T+dT/2, P2_inc, A,mDot);
P2_dec=P;
k=0;
pres= tolerance + 1.;
exception = false;
while (exception== false) && (pres>= tolerance) && k <= Max_iteration
err= errorEval_mach(hysolver, strInlet, T -dT/2, P2_dec, A, mDot,entr1 );
P2_dec = P2_dec +err;
if P2_dec< 0
exception = true;
else
...
[entr, enrg,momt,ro_dec, v_dec]=funcs(hysolver,strInlet, T-dT/2, P2_dec, A,mDot);
c= sqrt( abs( (P2_inc-P2_dec)*1000/(ro_inc- ro_dec)) ); % (m/s)
[entr, enrg,momt,ro,v] = funcs(hysolver,strInlet, T, P, A, mDot);
machNum = v / c;